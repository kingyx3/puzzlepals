...
describe('Achievement System', () => {
  ...
  describe('recordPuzzleCompletion', () => {
    it('should update player stats correctly', () => {
      ...
      expect(state.playerStats.totalTimeSpent).toBe(mockPuzzleStats.completionTime);
    ...
    it('should calculate averages correctly with multiple completions', () => {
      const firstStats = { ...mockPuzzleStats, completionTime: 10000, hintsUsed: 1 };
      const secondStats = { ...mockPuzzleStats, completionTime: 20000, hintsUsed: 3 };
  ...
  describe('checkAchievements', () => {
    it('should unlock first puzzle achievement', () => {
      const { recordPuzzleCompletion, checkAchievements } = useAchievementStore.getState();
      recordPuzzleCompletion(mockPuzzleStats);
  ...
  describe('getRecommendedDifficulty', () => {
    ...
    it('should recommend harder difficulty for good performance', () => {
      const { recordPuzzleCompletion, getRecommendedDifficulty } = useAchievementStore.getState();
      
      // Record several high-efficiency completions
      const highEfficiencyStats = { ...mockPuzzleStats, efficiency: 0.9, hintsUsed: 0 };
      
      for (let i = 0; i < 5; i++) {
        recordPuzzleCompletion({ ...highEfficiencyStats, puzzleId: `puzzle-${i}` });
      }
    ...
    it('should recommend easier difficulty for poor performance', () => {
      ...
      for (let i = 0; i < 5; i++) {
        recordPuzzleCompletion({ ...lowEfficiencyStats, puzzleId: `puzzle-${i}` });
      }
