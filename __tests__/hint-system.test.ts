...
describe('Hint System', () => {
  ...
  describe('piece classification', () => {
    it('should identify corner pieces correctly', () => {
      expect(isCornerPiece({ ...mockPiece, col: 0, row: 0 }, mockBoard)).toBe(
        true
      );
      expect(isCornerPiece({ ...mockPiece, col: 1, row: 1 }, mockBoard)).toBe(
        true
      );
      expect(isCornerPiece({ ...mockPiece, col: 0, row: 1 }, mockBoard)).toBe(
        true
      );
      expect(isCornerPiece({ ...mockPiece, col: 1, row: 0 }, mockBoard)).toBe(
        true
      );
    });
    ...
    it('should identify edge pieces correctly', () => {
      ...
      expect(isEdgePiece({ ...mockPiece, col: 0, row: 1 }, largerBoard)).toBe(
        true
      );
      expect(isEdgePiece({ ...mockPiece, col: 1, row: 0 }, largerBoard)).toBe(
        true
      );
      expect(isEdgePiece({ ...mockPiece, col: 1, row: 2 }, largerBoard)).toBe(
        true
      );
      expect(isEdgePiece({ ...mockPiece, col: 2, row: 1 }, largerBoard)).toBe(
        true
      );
      expect(isEdgePiece({ ...mockPiece, col: 1, row: 1 }, largerBoard)).toBe(
        false
      ); // interior piece
    });
  });
