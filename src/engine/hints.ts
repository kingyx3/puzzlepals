 ...
 */
export function suggestBestHint(board: BoardState, hintsUsed: number): HintResult {
...
export function findBestNextPiece(board: BoardState): Piece | null {
  ...
  const cornerPieces = unplacedPieces.filter(piece => isCornerPiece(piece, board));
  if (cornerPieces.length > 0) {
    return cornerPieces[0];
  }
  
  // Strategy 2: Prefer edge pieces
  const edgePieces = unplacedPieces.filter(piece => isEdgePiece(piece, board));
  if (edgePieces.length > 0) {
    return edgePieces[0];
  }
  
  // Strategy 3: Find piece closest to correct position
  return unplacedPieces.reduce((best, piece) => {
    const distanceToTarget = distance(
      getRectCenter({ x: piece.x, y: piece.y, width: piece.width, height: piece.height }),
      getRectCenter({ x: piece.targetX, y: piece.targetY, width: piece.width, height: piece.height })
    );
    
    const bestDistance = best ? distance(
      getRectCenter({ x: best.x, y: best.y, width: best.width, height: best.height }),
      getRectCenter({ x: best.targetX, y: best.targetY, width: best.width, height: best.height })
    ) : Infinity;
...
export function findEasiestPiece(board: BoardState): Piece | null {
  ...
  return unplacedPieces.reduce((easiest, piece) => {
    const distanceToTarget = distance(
      getRectCenter({ x: piece.x, y: piece.y, width: piece.width, height: piece.height }),
      getRectCenter({ x: piece.targetX, y: piece.targetY, width: piece.width, height: piece.height })
    );
    
    const easiestDistance = easiest ? distance(
      getRectCenter({ x: easiest.x, y: easiest.y, width: easiest.width, height: easiest.height }),
      getRectCenter({ x: easiest.targetX, y: easiest.targetY, width: easiest.width, height: easiest.height })
    ) : Infinity;
...
export function organizePiecesByType(board: BoardState): {
  ...
  return {
    ...
    edges: pieces.filter(piece => isEdgePiece(piece, board) && !isCornerPiece(piece, board)),
