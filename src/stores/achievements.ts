...
export const useAchievementStore = create<AchievementState>((set, get) => ({
  ...
  recordPuzzleCompletion: (stats) => {
    ...
    if (!currentStats) {
      console.warn(
        `Missing difficulty stats for ${stats.difficulty}, skipping operation`
      );
      return;
    }
    ...
    const updatedDifficultyStats = {
      ...
      [stats.difficulty]: {
        ...
        averageTime:
          (currentStats.averageTime * currentStats.completed +
            stats.completionTime) /
          newCompletionCount,
        bestTime:
          currentStats.bestTime === 0
            ? stats.completionTime
            : Math.min(currentStats.bestTime, stats.completionTime),
        averageHints:
          (currentStats.averageHints * currentStats.completed +
            stats.hintsUsed) /
          newCompletionCount,
      },
    };
    ...
    const streakContinues =
      !state.playerStats.lastPlayDate ||
    ...
    updatedPlayerStats.averageCompletionTime =
      updatedPlayerStats.totalTimeSpent /
      updatedPlayerStats.totalPuzzlesCompleted;
  ...
  checkAchievements: () => {
    ...
    if (
      ...
      console.warn(
        'Store state not properly initialized for checkAchievements, returning empty array'
      );
      return [];
    }
    ...
    state.achievements.forEach((achievement) => {
      ...
      switch (achievement.id) {
        ...
        case 'speed_demon':
          ...
          shouldUnlock = state.puzzleHistory.some(
            (puzzle) => puzzle.completionTime < 2 * 60 * 1000
          );
          break;
        ...
        case 'difficulty_explorer':
          ...
          const completedDifficulties = Object.values(
            state.playerStats.difficultyStats
  ...
  getRecommendedDifficulty: (): Difficulty => {
    ...
    if (!state || !state.playerStats || !state.puzzleHistory) {
      console.warn(
        'Store state not properly initialized in getRecommendedDifficulty'
      );
    ...
    const averageEfficiency =
      recentPuzzles.reduce((sum, puzzle) => sum + puzzle.efficiency, 0) /
      recentPuzzles.length;
    const averageHints =
      recentPuzzles.reduce((sum, puzzle) => sum + puzzle.hintsUsed, 0) /
      recentPuzzles.length;
    ...
    if (averageEfficiency > 0.8 && averageHints < 2) {
      ...
      return (
        difficultyProgression[currentMaxDifficulty as Difficulty] ||
        (currentMaxDifficulty as Difficulty)
      );
    }
  ...
  getEfficiencyScore: (completionTime, hintsUsed, difficulty) => {
    ...
    const difficultyMultipliers: Record<
      Difficulty,
      { timeTarget: number; hintPenalty: number }
    > = {
